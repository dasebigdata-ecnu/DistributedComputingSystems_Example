...
public class WindowWordCountTopology {
  public static void main(String[] args) throws Exception {
    ...
    // 设置Bolt取名"WINDOWCOUNT"，executor数量为2，任务数量为2，订阅策略为fieldsGrouping
    builder.setBolt("WINDOWCOUNT", new WindowBolt(), 2).fieldsGrouping("SPLIT", new Fields("word"));
    ...
    if (args[0].equals("cluster")) {
      // 在集群运行程序，拓扑的名称为WINDOWWORDCOUNT
      StormSubmitter.submitTopology("WINDOWWORDCOUNT", conf, builder.createTopology());
    } else if (args[0].equals("local")) {
      // 在本地IDE调试程序，拓扑的名称为WINDOWWORDCOUNT
      LocalCluster cluster = new LocalCluster();
      cluster.submitTopology("WINDOWWORDCOUNT", conf, builder.createTopology());
    } else {
      System.exit(-2);
    }
  }
